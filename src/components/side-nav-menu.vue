  ...
  export default {
    ...
    async setup(props, context) {
      ...
      onMounted(() => { 
        ...
        treeViewRef.value.instance.option('items', items);
        updateSelection();
      ...
      watch(
        ...
        () => {
          if (props.compactMode) {
            ...
            const aItems = await navigation.getUserMenus(app.appContext.config.globalProperties.$curUser)
        .then(aItems=>{
            for ( let item of aItems) {
              items.push({text:item.text,path:"/" + item.text,icon: "folder",selected: false, expanded: false,hasItems: true})
            }
        })
